...
nc_get_options(int argc, char **argv, struct instance *nci)
{
    ...
    for (;;) {
        ...
        case 't':
            test_conf = 1;//test conf for what?
            break;
 ...
 * returns false
 * use the conf_create to check weather the conf is valid
 */
static bool
...
nc_pre_run(struct instance *nci)
{
	...
	//update the pid of the sever instance
    nci->pid = getpid();
	...
	//doing nothing but logged
    nc_print_run(nci);

    return NC_OK;
}

//why u called exit as post run
//f**k indian...
static void
